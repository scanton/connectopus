	...
	renderServerAvatars(data) {
		if(data && data.length) {
			...
			for(let i in data) {
				...
				a.push( '<div class="server-avatar server-avatar-' + i + ' ' + (d.status == 'pending' ? 'contains-pending' : '') + '" data-id="' + d.id + '">' + icon + ' ' + d.name + ' <span class="server-status ' + d.status + '"></span><span class="glyphicon glyphicon-remove close-icon"></div>' );
				++count;
			}
